...
bool SerialPort::open(const std::string& port, unsigned int baud_rate) {
    ...
    try {
        ...
        std::cout << "[SerialPort] Opened " << port << " @ " << baud_rate << " baud" << std::endl;
        return true;
...
void SerialPort::close() {
    if (serial.is_open()) {
        try {
            serial.close();
            std::cout << "[SerialPort] Port closed" << std::endl;
        } catch (const std::exception& e) {
...
bool SerialPort::hasData() {
    ...
    try {
        serial.get_option(boost::asio::serial_port_base::bytes_to_read());
...
SerialPort::ArduinoStatus SerialPort::parseStatusMessage(const std::string& msg) {
    ...
    // Parse: status:running,mode:forward,load:25.50,scale:0.95,pos:100.50,105.20,98.70
    ...
    if (pos != std::string::npos) {
        ...
        size_t comma1 = pos_str.find(',');
        size_t comma2 = pos_str.find(',', comma1 + 1);
        
        status.pos1 = std::stof(pos_str.substr(0, comma1));
        status.pos2 = std::stof(pos_str.substr(comma1 + 1, comma2 - comma1 - 1));
        status.pos3 = std::stof(pos_str.substr(comma2 + 1));
    }
